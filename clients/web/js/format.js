...
$(function() {
    ...
    function showLocaleInfo() {
        ...
        $(document.body).append('<p>Latitude: ' + locale.latitude + '</p>').append('<p>Longitude: ' + locale.longitude + '</p>');
    }
    ...
    function pollTwitter(cb) {
        ...
        if (offline) {
            ...
            $.get(url + locale.latitude.replace('.', '_').replace('-', '*') + '/' + locale.longitude.replace('.', '_').replace('-', '*'), function(data) {
                // Typical response:  
        ...
        data.results.forEach(function(el, index) {
            if (el.geo != null) {
                ...
                if (geoIterator < 5) {
                    ...
                    link = '<a href="http://maps.google.com/maps?dirflg=w&daddr=' + destinationAddress + '&saddr=' + startAddress + '">' + el.text + '</a>';
    ...
    function bindFindBeerButton(state) {
        ...
        $('#find-beer').find('button').text(!state ? "Unavailable :(": "Find Beer Nao!").bind('click', function(e) {
